# Raid Signal v1.4.1 manual Windows test
...
2. Download `Raid-Signal-Setup-1.4.1.exe` and
   `Raid-Signal-Setup-1.4.1.exe.sha256` from the immutable
...
3. Open PowerShell in the download directory and verify the installer:
   ...
   Confirm the same hash is recorded in the release's `release.json`.
...
### Overview labels and quest details

1. At the initial map overview, enable extracts and confirm that names are
   readable on desktop and overlay without hovering or zooming. Check a crowded
   area and a map edge; use the overflow control to read full hidden names.
2. Change floors and extraction filters, and confirm hidden extracts stay hidden.
3. Click an active quest marker and a marker focused from Quest Navigator.
   Confirm that each opens its matching quest and objective, including when
   focusing changes maps or floors.
4. Open a possible-location marker. Confirm its location context and possible
   spawn wording remain visible.
5. Disconnect from the network. Confirm bundled descriptions and available quest
   artwork still load, generic artwork is labeled, and missing images have a
   useful fallback. Reconnect before sharing tests.
...
5. Confirm overview extraction labels and quest details work with touch.
6. Create two waypoints on one map and a waypoint on another map. Delete one
   individual waypoint, reload, and confirm it stays deleted.
7. Select the remaining waypoint, then use **Clear waypoints on this map**.
   Confirm its selection and popup close, reload, and check the other map's
   waypoint remains. Confirm built-in map markers remain intact.
8. Repeat removal with keyboard navigation and check panning, zooming, and
   waypoint creation still work.
9. Click **End Session** and confirm that the phone disconnects.
...
- The upgrade retains settings and Windows reports version 1.4.1.
- Overview extraction names and full-name access work on all three surfaces.
- Quest markers show the correct offline details and attributed artwork.
- Companion deletion persists and current-map clearing preserves other maps.
